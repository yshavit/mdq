...
impl WordsBuffer {
    ...
    pub fn drain_pending_word(&mut self, mut drain_action: impl FnMut(char)) {
    ...
    fn start_new_line(&mut self, mut drain_action: impl FnMut(char)) {
        drain_action('\n');
        self.reset();
    }

    fn drain_without_leading_space(&mut self, mut drain_action: impl FnMut(char)) {
...
mod tests {
    ...
    mod boundary_is_never {
        ...
        fn never_whitespace_and_no_limit() {
            ...
            wb.drain_pending_word(|ch| last_seen = Some(ch));
    ...
    impl WbHelper {
        ...
        pub fn end(mut self) -> String {
            self.wb.drain_pending_word(|ch| self.s.push(ch));
            self.s
        }
    }
}
